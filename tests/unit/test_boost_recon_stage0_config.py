...
def test_blob_exists_returns_true_when_blob_exists() -> None:
    ...
    with patch("batch_live_reconciliation_service.stages.stage0_config_pull.get_storage_client", return_value=mock_client):
...
def test_run_stage0_dry_run_no_gcs_calls() -> None:
    config = _make_config()
    with patch("batch_live_reconciliation_service.stages.stage0_config_pull.get_storage_client") as mock_gcs, \
...
def test_run_stage0_passes_when_all_blobs_present() -> None:
    ...
    with patch("batch_live_reconciliation_service.stages.stage0_config_pull._blob_exists", side_effect=_blob_true), \
         patch("batch_live_reconciliation_service.stages.stage0_config_pull.get_storage_client", return_value=mock_client), \
         patch("batch_live_reconciliation_service.stages.stage0_config_pull.log_event"):
...
def test_run_stage0_fails_when_blobs_missing() -> None:
    ...
    with patch("batch_live_reconciliation_service.stages.stage0_config_pull._blob_exists", return_value=False), \
         patch("batch_live_reconciliation_service.stages.stage0_config_pull.log_event"):
        result = run_stage0(config, "2026-03-11", dry_run=False)
...
def test_run_stage0_fails_on_config_snapshot_missing() -> None:
    ...
    with patch("batch_live_reconciliation_service.stages.stage0_config_pull._blob_exists", return_value=True), \
         patch("batch_live_reconciliation_service.stages.stage0_config_pull.get_storage_client", return_value=mock_client), \
         patch("batch_live_reconciliation_service.stages.stage0_config_pull.log_event"):
        result = run_stage0(config, "2026-03-11", dry_run=False)
