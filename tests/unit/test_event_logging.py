# Canonical template applied — see unified-trading-codex/06-coding-standards/test-templates/
...
def test_error_events_exist(all_event_markers: set[str]) -> None:
    ...
    if not all_event_markers:
        pytest.skip("No event markers found in source — check service directory")
    error_events = {"FAILED", "SERVICE_ERROR", "VALIDATION_FAILED", "ERROR", "ALERT_FAILED"}
